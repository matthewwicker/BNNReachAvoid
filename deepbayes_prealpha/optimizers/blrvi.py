...
from deepbayes_prealpha.optimizers import optimizer 
from deepbayes_prealpha.optimizers import losses
from deepbayes_prealpha import analyzers
    ...
    def step(self, features, labels, lrate):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            if(int(self.robust_train) == 0):
                ...
                #IBP(model, inp, weights, eps, predict=False):
                #print("Features: ", type(features), features.shape)
                logit_l, logit_u = analyzers.IBP_learning(model=self, inp=features, weights=self.model.trainable_variables, eps=self.epsilon, predict=False)
                logit_u = tf.cast(logit_u, dtype=tf.float32)
                worst_case = self.model.layers[-1].activation(logit_u)
                output = (self.robust_lambda * predictions) + ((1-self.robust_lambda) * worst_case)
                loss =  self.loss_func(labels, output)
                ...
                loss = self.loss_func(labels, output_u)  + (0.025*tf.keras.metrics.mean_absolute_error(output_l, output_u))
                #print(tf.reduce_mean(self.loss_func(labels, output_u)), tf.reduce_mean(tf.keras.metrics.mean_squared_error(output_l, output_u)))
...
#        print(g)
    ...
    def sample(self):
        ...
        for i in range(len(self.posterior_mean)):
            var = tf.math.reciprocal(tf.math.sqrt(self.N*self.posterior_var[i]))
            sampled_weights.append(np.random.normal(loc=self.posterior_mean[i], scale=var))
