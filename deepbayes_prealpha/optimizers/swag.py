...
class StochasticWeightAveragingGaussian(optimizer.Optimizer):
    ...
    def step(self, features, labels, lrate):
        ...
        #self.train_rob(labels, worst_case)
        #print("LENGTH OF STACK: ", len(self.weights_stack))
    ...
    def get_posterior(self):
        """
        w_ex = self.weights_stack[-1]
        mean, var = [], []
        for i in range(len(w_ex)):
            vals = []
            for n in range(250):
                vals.append(self.weights_stack[-1*n][i])
            var.append(tf.Tensor(np.var(vals, axis=0)))
            mean.append(tf.Tensor(np.mean(vals, axis=0)))
        self.posterior_var = tf.Tensor(var)
        self.posterior_mean = tf.Tensor(mean)
        """
        ws = self.weights_stack[-250:]
